...
$db = db();
$rows = $db->query('SELECT executive_id, full_name, mobile_no, created_at FROM executives ORDER BY created_at DESC')->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="nav">
        <div>SRMS EHEALTH - Executives</div>
        <a href="view_analysis.php">Back to Analysis</a>
    </div>
    <div class="page">
        <div class="title">Executives</div>
        <div class="card">
            <table>
                <thead>
                    <tr><th>Executive ID</th><th>Name</th><th>Mobile</th><th>Registered</th></tr>
                </thead>
                <tbody>
                <?php if (empty($rows)): ?>
                    <tr><td colspan="4">No executives found.</td></tr>
                <?php else: ?>
                    <?php foreach ($rows as $r): ?>
                        <tr>
                            <td><?= htmlspecialchars($r['executive_id']) ?></td>
