...
if (is_logged_in() && get_session_role() === 'pharmacy_staff') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $login    = trim($_POST['pharmacyId'] ?? '');
    $password = $_POST['password'] ?? '';

    if (!$login || !$password) {
        $error = 'Pharmacy ID / Mobile and password are required.';
    } else {
        $db = db();
        ensure_pharmacy_staff_table($db);
        $st = $db->prepare(
            'SELECT id, pharmacy_id, full_name, password
             FROM pharmacy_staff
             WHERE pharmacy_id = ? OR mobile_no = ?
             LIMIT 1'
        );
        ...
        if ($row) {
            ...
            if (!$isValid && strcasecmp($password, $row['pharmacy_id']) === 0) {
                $newHash = hash_password($row['pharmacy_id']);
                $up = $db->prepare('UPDATE pharmacy_staff SET password = ? WHERE id = ? LIMIT 1');
        ...
        if ($isValid) {
            ...
            $error = 'Invalid Pharmacy ID / Mobile or password.';
        }
    }
}
?>
...
<head>
    ...
    <title>Pharmacy Staff Login - eHealth</title>
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .card { background:white; border-radius:18px; box-shadow:0 20px 60px rgba(0,0,0,0.25); width:100%; max-width:460px; padding:48px 44px 40px; }
...
<div class="page-body">
    <div class="card">
        <div class="card-header">
            ...
            <p>Access your pharmacy dashboard</p>
        </div>

        <div class="alert alert-info">
            First time logging in? Use your <strong>Pharmacy ID</strong> as your password (e.g. <code>P202603001</code>)
        </div>
        ...
        <form method="POST">
            <div class="form-group">
                <label>Pharmacy ID / Mobile Number <span class="required">*</span></label>
                <div class="input-wrapper">
                    <input type="text" name="pharmacyId"
                           value="<?= htmlspecialchars($_POST['pharmacyId'] ?? '') ?>"
                           placeholder="e.g. P202603001 or mobile number"
