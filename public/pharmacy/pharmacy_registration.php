...
// ── Generate next Pharmacy ID (format: P{YYYY}{MM}{NNN}) ──
function generate_pharmacy_id(mysqli $db): string {
    $year   = date('Y');
    $month  = date('m');
    $prefix = 'P' . $year . $month;
    $like   = $prefix . '%';

    $stmt = $db->prepare('SELECT pharmacy_id FROM pharmacy_staff WHERE pharmacy_id LIKE ? ORDER BY pharmacy_id DESC LIMIT 1');
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    $next = $row ? ((int) substr($row['pharmacy_id'], -3)) + 1 : 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['logout'])) {
    ...
    if (!$full_name || !$gender || !$dob || !$qualification || !$mobile || !$email || !$aadhar || !$pincode || !$state || !$district) {
        ...
        $db   = db();
        ensure_pharmacy_staff_table($db);
        $stmt = $db->prepare('SELECT id FROM pharmacy_staff WHERE mobile_no = ? OR email = ? OR aadhar_no = ? LIMIT 1');
        ...
        if ($exists) {
            $error = 'A pharmacy staff member with this mobile, email, or Aadhar already exists.';
        } else {
            $pharmacy_id = generate_pharmacy_id($db);
            $raw_password = $pharmacy_id;
            $hashed_pw    = hash_password($raw_password);

            $admin_id = get_session_id();
            $stmt = $db->prepare('
                INSERT INTO pharmacy_staff
                    (pharmacy_id, full_name, gender, dob, qualification,
            ...
            if ($stmt->execute()) {
                $success = "Pharmacy staff registered successfully!<br>
                    &nbsp;👤 <strong>Pharmacy ID:</strong> {$pharmacy_id}<br>
                    &nbsp;🔑 <strong>Default Password:</strong> {$pharmacy_id}<br>
                    &nbsp;📱 Share these credentials with the pharmacy staff member. They can change their password after first login.";
                $modal_data = [
...
<head>
    ...
    <title>Pharmacy Staff Registration - eHealth</title>
    <style>
...
<div class="card">
    ...
    <div class="card-title">
        <h1>🧑‍💼 Pharmacy Staff Registration</h1>
        <p>Pharmacy — Admin Registration</p>
    </div>
    ...
    <form method="POST" novalidate>
        ...
        <div class="form-row full" id="qualOtherWrap" style="display:none;">
            <div class="form-group">
                ...
                <span class="hint">Provide the exact pharmacy qualification if it is not listed above.</span>
            </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" placeholder="pharmacy@email.com"
        ...
        <button type="submit" class="btn-submit">REGISTER PHARMACY STAFF</button>
...
<div class="modal-overlay show" id="successModal">
    <div class="modal-card">
        <div class="modal-header">
            ...
            <h2>Pharmacy Staff Registered Successfully</h2>
        </div>
        <div class="modal-body">
            <div class="modal-id-badge">
                <div class="modal-id-label">Pharmacy ID &amp; Default Password</div>
                <div class="modal-id-value"><?= htmlspecialchars($modal_data['pharmacy_id']) ?></div>
            </div>
