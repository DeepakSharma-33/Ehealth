...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$current || !$new || !$confirm) {
        ...
        $patientNumericId = get_session_id();
        $db = db();
        $st = $db->prepare('SELECT password FROM patients WHERE id = ? LIMIT 1');
        $st->bind_param('i', $patientNumericId);
        $st->execute();
        ...
        if (!$row) {
            ...
            $up = $db->prepare('UPDATE patients SET password = ? WHERE id = ?');
            $up->bind_param('si', $hashed, $patientNumericId);
            $up->execute();
            $up->close();
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:'DM Sans', sans-serif; background:#d8f3dc; min-height:100vh; display:flex; flex-direction:column; }
        .nav { background:#2d6a4f; color:#fff; height:56px; padding:0 28px; display:flex; align-items:center; justify-content:space-between; }
        ...
        .btn { width:100%; padding:12px; background:#2d6a4f; color:#fff; border:none; border-radius:8px; font-weight:700; cursor:pointer; }
