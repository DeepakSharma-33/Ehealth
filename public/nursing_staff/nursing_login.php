...
// Already logged in as nursing staff → go to dashboard
if (is_logged_in() && get_session_role() === 'nursing_staff') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $login    = trim($_POST['nursingId'] ?? '');
    $password = $_POST['password'] ?? '';   // do NOT trim — passwords may have spaces

    if (!$login || !$password) {
        $error = 'Nursing ID / Mobile and password are required.';
    } else {
        $db = db();
        ensure_nursing_staff_table($db);
        $st = $db->prepare(
            'SELECT id, nursing_id, full_name, password
             FROM nursing_staff
             WHERE nursing_id = ? OR mobile_no = ?
             LIMIT 1'
        );
        ...
        if ($row) {
            ...
            // 2. First-time fallback: nursing staff types their Nursing ID as password.
            //    Registration stored a random hash they may never have seen.
            //    Accept Nursing ID as password and re-hash it for future logins.
            if (!$isValid && strcasecmp($password, $row['nursing_id']) === 0) {
                $newHash = hash_password($row['nursing_id']);
                $up = $db->prepare('UPDATE nursing_staff SET password = ? WHERE id = ? LIMIT 1');
        ...
        if ($isValid) {
            ...
            // Store nursing_id string separately for dashboard display / business logic
            $_SESSION['nursing_id'] = $row['nursing_id'];

            header('Location: nursing_dashboard.php');
            exit;
        } else {
            $error = 'Invalid Nursing ID / Mobile or password.';
        }
    }
}
?>
...
<head>
    ...
    <title>Nursing Staff Login - eHealth</title>
    <style>
...
<div class="page-body">
    <div class="card">
        ...
        <div class="card-header">
            ...
            <h1>Nursing Staff Login</h1>
            <p>Access your eHealth nursing staff dashboard</p>
        </div>

        <div class="alert alert-info">
            💡 <strong>First time logging in?</strong> Use your <strong>Nursing ID</strong> as your password (e.g. <code>N202603001</code>)
        </div>
        ...
        <form method="POST">
            <div class="form-group">
                <label>Nursing ID / Mobile Number <span class="required">*</span></label>
                <div class="input-wrapper">
                    <input type="text" name="nursingId"
                           value="<?= htmlspecialchars($_POST['nursingId'] ?? '') ?>"
                           placeholder="e.g. N202603001 or mobile number"
