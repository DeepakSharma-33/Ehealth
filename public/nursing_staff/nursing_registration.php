...
// ── Generate next Nursing ID (format: N{YYYY}{MM}{NNN}) ──
function generate_nursing_id(mysqli $db): string {
    $year   = date('Y');
    $month  = date('m');
    $prefix = 'N' . $year . $month;
    $like   = $prefix . '%';

    $stmt = $db->prepare('SELECT nursing_id FROM nursing_staff WHERE nursing_id LIKE ? ORDER BY nursing_id DESC LIMIT 1');
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    $next = $row ? ((int) substr($row['nursing_id'], -3)) + 1 : 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['logout'])) {
    ...
    if (!$full_name || !$gender || !$dob || !$qualification || !$mobile || !$email || !$aadhar || !$pincode || !$state || !$district) {
        ...
        $db   = db();
        ensure_nursing_staff_table($db);
        $stmt = $db->prepare('SELECT id FROM nursing_staff WHERE mobile_no = ? OR email = ? OR aadhar_no = ? LIMIT 1');
        ...
        if ($exists) {
            $error = 'A nursing staff member with this mobile, email, or Aadhar already exists.';
        } else {
            $nursing_id = generate_nursing_id($db);
            $raw_password = $nursing_id;
            $hashed_pw    = hash_password($raw_password);

            $admin_id = get_session_id();
            $stmt = $db->prepare('
                INSERT INTO nursing_staff
                    (nursing_id, full_name, gender, dob, qualification,
            ...
            if ($stmt->execute()) {
                $success = "Nursing staff registered successfully!<br>
                    &nbsp;👤 <strong>Nursing ID:</strong> {$nursing_id}<br>
                    &nbsp;🔑 <strong>Default Password:</strong> {$nursing_id}<br>
                    &nbsp;📱 Share these credentials with the nursing staff member. They can change their password after first login.";
                $modal_data = [
...
<head>
    ...
    <title>Nursing Staff Registration - eHealth</title>
    <style>
...
<div class="card">
    ...
    <div class="card-title">
        <h1>🧑‍💼 Nursing Staff Registration</h1>
    ...
    <form method="POST" novalidate>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="qualification" id="qualificationSelect" required>
                    ...
                    <?php foreach (['ANM','GNM','B.Sc Nursing','Post Basic B.Sc Nursing','M.Sc Nursing','PhD Nursing','Nursing Diploma','Other'] as $q): ?>
        ...
        <div class="form-row full" id="qualOtherWrap" style="display:none;">
            <div class="form-group">
                ...
                <span class="hint">Provide the exact nursing qualification if it is not listed above.</span>
            </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" placeholder="nursing@email.com"
        ...
        <button type="submit" class="btn-submit">REGISTER NURSING STAFF</button>
...
<div class="modal-overlay show" id="successModal">
    <div class="modal-card">
        <div class="modal-header">
            ...
            <h2>Nursing Staff Registered Successfully</h2>
        </div>
        <div class="modal-body">
            <div class="modal-id-badge">
                <div class="modal-id-label">Nursing ID &amp; Default Password</div>
                <div class="modal-id-value"><?= htmlspecialchars($modal_data['nursing_id']) ?></div>
            </div>
