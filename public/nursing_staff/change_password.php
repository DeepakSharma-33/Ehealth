...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$current || !$new || !$confirm) {
        ...
        $nurseId = get_session_id();
        $db = db();
        $st = $db->prepare('SELECT password FROM nursing_staff WHERE id = ? LIMIT 1');
        $st->bind_param('i', $nurseId);
        $st->execute();
        $row = $st->get_result()->fetch_assoc();
        $st->close();

        if (!$row) {
            $error = 'Nursing staff account not found.';
        } elseif (!verify_password($current, $row['password'])) {
            $error = 'Current password is incorrect.';
        } else {
            $hashed = hash_password($new);
            $up = $db->prepare('UPDATE nursing_staff SET password = ? WHERE id = ?');
            $up->bind_param('si', $hashed, $nurseId);
            $up->execute();
            $up->close();
...
<head>
    ...
    <title>Change Password - Nursing Staff</title>
    <style>
...
<body>
    <div class="nav">
        <div>SRMS EHEALTH - Nursing Staff</div>
        <a href="nursing_dashboard.php">Back to Dashboard</a>
    </div>
    <div class="page">
        <div class="card">
