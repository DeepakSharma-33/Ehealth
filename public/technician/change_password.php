...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$current || !$new || !$confirm) {
        ...
        $techId = get_session_id();
        $db = db();
        $st = $db->prepare('SELECT password FROM technicians WHERE id = ? LIMIT 1');
        $st->bind_param('i', $techId);
        $st->execute();
        ...
        if (!$row) {
            ...
            $up = $db->prepare('UPDATE technicians SET password = ? WHERE id = ?');
            $up->bind_param('si', $hashed, $techId);
            $up->execute();
            $up->close();
...
<head>
    ...
    <title>Change Password - Technician</title>
    <style>
...
<body>
    <div class="nav">
        <div>SRMS EHEALTH - Technician</div>
