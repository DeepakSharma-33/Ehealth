...
// ── Generate next Technician ID (format: T{YYYY}{MM}{NNN}) ──
function generate_technician_id(mysqli $db): string {
    $year   = date('Y');
    $month  = date('m');
    $prefix = 'T' . $year . $month;
    $like   = $prefix . '%';

    $stmt = $db->prepare('SELECT technician_id FROM technicians WHERE technician_id LIKE ? ORDER BY technician_id DESC LIMIT 1');
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    $next = $row ? ((int) substr($row['technician_id'], -3)) + 1 : 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['logout'])) {
    ...
    if (!$full_name || !$gender || !$dob || !$qualification || !$mobile || !$email || !$aadhar || !$pincode || !$state || !$district) {
        ...
        $stmt = $db->prepare('SELECT id FROM technicians WHERE mobile_no = ? OR email = ? OR aadhar_no = ? LIMIT 1');
        ...
        if ($exists) {
            $error = 'A technician with this mobile, email, or Aadhar already exists.';
        } else {
            $technician_id = generate_technician_id($db);
            $raw_password = $technician_id;
            $hashed_pw    = hash_password($raw_password);

            $admin_id = get_session_id();
            $stmt = $db->prepare('
                INSERT INTO technicians
                    (technician_id, full_name, gender, dob, qualification,
            ...
            if ($stmt->execute()) {
                $success = "Technician registered successfully!<br>
                    &nbsp;👤 <strong>Technician ID:</strong> {$technician_id}<br>
                    &nbsp;🔑 <strong>Default Password:</strong> {$technician_id}<br>
                    &nbsp;📱 Share these credentials with the technician. They can change their password after first login.";
                $modal_data = [
...
<head>
    ...
    <title>Technician Registration - eHealth</title>
    <style>
    ...
    <form method="POST" novalidate>
        ...
        <div class="form-row full" id="qualOtherWrap" style="display:none;">
            <div class="form-group">
                ...
                <span class="hint">Provide the exact technician qualification if it is not listed above.</span>
            </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" placeholder="technician@email.com"
        ...
        <button type="submit" class="btn-submit">REGISTER TECHNICIAN</button>
...
<div class="modal-overlay show" id="successModal">
    <div class="modal-card">
        <div class="modal-header">
            ...
            <h2>Technician Registered Successfully</h2>
        </div>
        <div class="modal-body">
            <div class="modal-id-badge">
                <div class="modal-id-label">Technician ID &amp; Default Password</div>
                <div class="modal-id-value"><?= htmlspecialchars($modal_data['technician_id']) ?></div>
            </div>
